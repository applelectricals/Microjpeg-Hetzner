...
// Operations log types (legacy)
export type OperationsLogEntry = typeof operationsLog.$inferSelect;
export type InsertOperationsLogEntry = typeof operationsLog.$inferInsert;
...
// Operation log for audit trail with superuser bypass tracking
export const operationLog = pgTable("operation_log", {
  ...
  // Superuser bypass tracking
  wasBypassed: boolean("was_bypassed").default(false),
  bypassReason: text("bypass_reason"),
  actedByAdminId: varchar("acted_by_admin_id").references(() => users.id),
  timestamp: timestamp("timestamp").defaultNow(),
});

export type UserUsage = typeof userUsage.$inferSelect;
export type InsertUserUsage = typeof userUsage.$inferInsert;
export type OperationLogEntry = typeof operationLog.$inferSelect;
export type InsertOperationLogEntry = typeof operationLog.$inferInsert;

// Admin audit logs for tracking superuser actions
export const adminAuditLogs = pgTable("admin_audit_logs", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  adminUserId: varchar("admin_user_id").references(() => users.id).notNull(),
  action: varchar("action").notNull(), // COUNTERS_RESET, BYPASS_TOGGLED, ENFORCEMENT_CHANGED
  targetUserId: varchar("target_user_id").references(() => users.id),
  targetSessionId: varchar("target_session_id"),
...
// App settings for global counter enforcement and feature flags
export const appSettings = pgTable("app_settings", {
  id: serial("id").primaryKey(),
  countersEnforcement: jsonb("counters_enforcement").default('{"hourly":true,"daily":true,"monthly":true}').notNull(),
  adminUiEnabled: boolean("admin_ui_enabled").default(false),
  superuserBypassEnabled: boolean("superuser_bypass_enabled").default(false),
  updatedAt: timestamp("updated_at").defaultNow(),
...
export const adminCounterActionSchema = z.object({
  scope: z.enum(["hourly", "daily", "monthly", "all"]),
  target: z.enum(["user", "session", "all"]),
  userId: z.string().optional(),
  sessionId: z.string().optional(),
  pageIdentifier: z.string().optional(),
});

export const adminEnforcementToggleSchema = z.object({
  hourly: z.boolean().optional(),
  daily: z.boolean().optional(),
  monthly: z.boolean().optional(),
});

export const adminBypassSessionSchema = z.object({
  enabled: z.boolean(),
  reason: z.string().optional(),
});

export type SuperuserCreateInput = z.infer<typeof superuserCreateSchema>;
export type AdminCounterAction = z.infer<typeof adminCounterActionSchema>;
export type AdminEnforcementToggle = z.infer<typeof adminEnforcementToggleSchema>;
export type AdminBypassSession = z.infer<typeof adminBypassSessionSchema>;
