...
const PAGE_IDENTIFIER = 'premium-29'; // Premium compression page
...
export default function MicroJPEGLanding() {
  ...
  // Subscription guard - check if user has premium access
  const [showPaymentModal, setShowPaymentModal] = useState(false);
  const hasPremiumAccess = subscriptionStatus === 'active' && isPremium;
  
  useEffect(() => {
    if (!isLoading && isAuthenticated && !hasPremiumAccess) {
      setShowPaymentModal(true);
    }
  }, [isAuthenticated, isLoading, hasPremiumAccess]);
  ...
  const startProcessing = useCallback(async () => {
    ...
    const hourlyCheck = canOperateHourly(filesToProcess.length, 100);
    ...
    try {
      ...
      formData.append('pageSource', 'premium');
      ...
      if (data.results && data.results.length > 0) {
        ...
        console.log(`Recorded ${operationCount} operations for ${data.results.length} files processed (premium page)`);
      }
  ...
  const processSpecificFormat = useCallback(async (format: string) => {
    ...
    try {
      ...
      if (data.results && data.results.length > 0) {
        ...
        console.log(`Recorded ${operationCount} operations for ${data.results.length} files converted to ${format} (premium page)`);
      }
  ...
  const handleFiles = useCallback((files: FileList | File[]) => {
    ...
    // PREMIUM PAGE: Fixed 3 concurrent uploads limit
    const maxBatchSize = 3;
    if (fileArray.length > maxBatchSize) {
      toast({
        title: "Batch upload limit reached",
        description: `Premium users can upload up to ${maxBatchSize} files at a time.`,
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="min-h-screen hero-background-premium relative overflow-hidden">
      <SEOHead
        title="Premium Image Compression - Micro JPEG"
        description="Professional image compression with advanced features. Unlimited file sizes, priority processing, multiple output formats, and batch processing. Perfect for professionals and businesses."
        canonicalUrl="https://microjpeg.com/premium"
        keywords="premium image compression, professional image optimizer, unlimited file size, batch processing, priority support"
      />
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <h1 className="text-3xl lg:text-5xl font-bold font-poppins text-brand-dark leading-tight">
                  Ready for More?<br />
                  <span style={{color: '#AD0000'}}>Enterprise Unleashes True Power.</span><br />
                ...
                <p className="text-lg text-brand-dark/80 font-opensans max-w-md">
                  While Premium gives you solid capabilities, Enterprise delivers <span style={{color: '#AD0000'}} className="font-semibold">game-changing scale</span>. 
                  4x bigger files, 5x more operations, unlimited concurrent processing. 
                  <span style={{color: '#AD0000'}} className="font-semibold">Stop hitting limits. Start scaling.</span>
                </p>
                
                <div className="flex flex-col sm:flex-row gap-4">
                  <Button 
                    size="lg" 
                    onClick={() => window.location.href = '/subscribe?plan=enterprise'}
                    style={{backgroundColor: '#AD0000'}} 
                    className="hover:opacity-90 text-white font-semibold px-8 py-3 rounded-lg"
                  >
                    Scale to Enterprise Now
            ...
            <Card className="p-6 text-center bg-white shadow-lg">
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div 
                  ...
                  style={{ width: `${Math.min(((usageStats?.regular?.monthly?.used || 0) / 10000) * 100, 100)}%` }}
                ></div>
              </div>
              <div className="text-xs text-gray-500 mt-2">
                {(10000 - (usageStats?.regular?.monthly?.used || 0)).toLocaleString()} operations remaining
              </div>
            </Card>
