...
async function generatePage(browser, pageConfig) {
  ...
  try {
    ...
    // Wait for content
    // Wait for Footer (Critical for Link Mesh)
    try {
      await page.waitForSelector('footer', { timeout: 15000 });
      console.log('   ✅ Footer found');
    } catch (e) {
      console.log('   ⚠️ Footer NOT found - page might be incomplete');
    }

    // Wait for Link Mesh (ensure lots of links > 20)
    try {
      await page.waitForFunction(() => document.querySelectorAll('a').length > 20, { timeout: 10000 });
      const linkCount = await page.evaluate(() => document.querySelectorAll('a').length);
      console.log(`   ✅ Link mesh found (${linkCount} links)`);
    } catch (e) {
      console.log('   ⚠️ Link mesh missing or too small');
    }

    // Explicit small delay for final hydration
